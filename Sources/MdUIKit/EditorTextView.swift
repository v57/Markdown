...
public final class EditorTextView: UITextView, UITextViewDelegate, NSLayoutManagerDelegate {
    ...
    public init() {
        ...
        markdownContainer = NSTextContainer(size: CGSize(width: 0, height: CGFloat.greatestFiniteMagnitude))
    ...
    public func textView(_ textView: UITextView, shouldInteractWith url: URL, in characterRange: NSRange,
    ...
    @objc private func handleTap(_ recognizer: UITapGestureRecognizer) {
        ...
        guard index < ns.length,
              storage.attribute(.markdownCheckbox, at: index, effectiveRange: nil) != nil else { return }
    ...
    private func characterIndexForTap(at point: CGPoint) -> Int {
        let pointInContainer = CGPoint(x: point.x - textContainerInset.left,
                                       y: point.y - textContainerInset.top)
        var fraction: CGFloat = 0
        return layoutManager.characterIndex(for: pointInContainer,
                                            in: textContainer,
