    ...
    public static func image(checked: Bool, size: CGFloat) -> NSImage {
      if checked {
        NSImage(systemSymbolName: "checkmark.circle.fill", accessibilityDescription: "check")!
          .withSymbolConfiguration(.init(pointSize: size, weight: .semibold).applying(.init(paletteColors: [.white, .systemBlue])))!
      } else {
        NSImage(systemSymbolName: "circle", accessibilityDescription: "uncheck")!
          .withSymbolConfiguration(.init(pointSize: size, weight: .regular).applying(.init(paletteColors: [.systemBlue])))!
      }
    }
}
#endif
