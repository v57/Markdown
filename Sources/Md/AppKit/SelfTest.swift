  ...
  @MainActor public enum SelfTest {
    ...
    public static func headlessSequenceProbe() {
      ...
      final class RestyleDelegate: NSObject, NSTextStorageDelegate {
        ...
        func textStorage(
          ...
          ) { attrs, range, _ in textStorage.setAttributes(attrs, range: range)  // NO begin/endEditing: fires per-run, still inside the callback
          }
      ...
      func step2(_ label: String) {
        ...
        var frags: [String] = []
        lm2.enumerateLineFragments(
          forGlyphRange: lm2.glyphRange(
            forCharacterRange: NSRange(location: 0, length: storage2.length),
            actualCharacterRange: nil)
        ) { _, _, _, gr, _ in
          frags.append("[" + String(gr.location) + "," + String(NSMaxRange(gr)) + ")")
        }
        print(
          "SEQPROBE2 " + label + " len=" + String(storage2.length) + " frags="
      ...
      func type2(_ s: String) {
        ...
        lm2.invalidateGlyphs(
          forCharacterRange: affected, changeInLength: 0, actualCharacterRange: nil)
    ...
    public static func typingProbe() {
      ...
      DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
        ...
        report(
          "string verbatim", storage.string == expected,
          String(format: "got [%@] len=%d", storage.string, ns.length))
        var syntax: [Int] = []
        for i in 0..<ns.length {
          if storage.attribute(.markdownSyntax, at: i, effectiveRange: nil) != nil {
            syntax.append(i)
          }
        }
        ...
        let gAll = lm.glyphRange(
          forCharacterRange: NSRange(location: 0, length: ns.length), actualCharacterRange: nil)
        var parts: [String] = []
        ...
        while g < NSMaxRange(gAll) {
          ...
          parts.append(
            String(format: "g%d=ch%d gl%d x%.1f a%.1f", g, ci, gl, Double(loc.x), adv ?? -1))
          prevX = loc.x
          g += 1
        }
        ...
        lm.enumerateLineFragments(forGlyphRange: gAll) { rect, used, _, glyphRange, _ in
          frags.append(
            String(
              format: "g[%d,%d) w%.1f y%.1f", glyphRange.location, NSMaxRange(glyphRange),
              used.width, rect.minY))
        }
        ...
        let breakOK =
          fragRanges.count == 2 && fragRanges[0] == NSRange(location: 0, length: 6)
          && fragRanges[1] == NSRange(location: 6, length: 5)
        report(
          "second line keeps its first char", breakOK,
          fragRanges.map { "[" + String($0.location) + "," + String(NSMaxRange($0)) + ")" }.joined(
            separator: " "))
        let gH1 = lm.glyphRange(
          forCharacterRange: NSRange(location: 0, length: 1), actualCharacterRange: nil)
        let gH2 = lm.glyphRange(
        ...
        report(
          "second-line H keeps its glyph", gl1 != 0 && gl1 == gl2,
    ...
    public static func scrollGeometryProbe() {
      ...
      func measure(_ label: String) {
        ...
        print(
          "SCROLLPROBE \(label): frameH=\(frameH) usedH=\(used.height) expected=\(expected) delta=\(delta) contentSize=\(scroll.contentSize)"
        )
        report("frame tracks usedRect", abs(delta) < 1.0, "delta=\(delta)")
      }
      print(
        "SCROLLPROBE lmDelegateIsTV=\((tv.layoutManager?.delegate as AnyObject?) === tv) minSize=\(tv.minSize) maxSize=\(tv.maxSize) autoresizing=\(tv.autoresizingMask.rawValue)"
      )
    ...
    public static func editPathProbe() {
      ...
      if headingRange.location != NSNotFound {
        func headingFrags() -> String {
          tv.layoutManager?.ensureLayout(for: tv.textContainer!)
          let g = tv.layoutManager!.glyphRange(
            forCharacterRange: headingRange, actualCharacterRange: nil)
          var frags: [String] = []
        ...
        tv.setSelectedRange(
          NSRange(location: headingRange.location + headingRange.length, length: 0))
        let dActive = headingFrags()
        let stable = dInactive == dActive
        print(
          "EDITPROBE \(stable ? "PASS" : "FAIL") heading line stable: inactive=[\(dInactive)] active=[\(dActive)]"
        )
      }
      ...
      report(
        "deleteBackward", !tv.string.hasSuffix("Z") && afterDelete == startLen,
        "len \(afterInsert) -> \(afterDelete)")
      ...
      tv.insertNewline(nil)
      report(
        "list continuation marker", tv.string.hasSuffix("- [ ] task\n- [ ] "),
        "suffix=[\(String(tv.string.suffix(24)))]")
      report(
        "list continuation caret", tv.selectedRange.location == (tv.string as NSString).length,
        "caret=\(tv.selectedRange.location) len=\((tv.string as NSString).length)")
      tv.insertNewline(nil)
      report(
        "list continuation exit", tv.string.hasSuffix("- [ ] task\n"),
        "suffix=[\(String(tv.string.suffix(16)))]")
      report(
        "list continuation exit caret", tv.selectedRange.location == (tv.string as NSString).length,
        "caret=\(tv.selectedRange.location) len=\((tv.string as NSString).length)")
      ...
      tv.insertNewline(nil)
      report(
        "quote continuation marker", tv.string.hasSuffix("> quoted\n> "),
        "suffix=[\(String(tv.string.suffix(18)))]")
      report(
        "quote continuation caret", tv.selectedRange.location == (tv.string as NSString).length,
        "caret=\(tv.selectedRange.location) len=\((tv.string as NSString).length)")
      tv.insertNewline(nil)
      report(
        "quote continuation exit", tv.string.hasSuffix("> quoted\n"),
        "suffix=[\(String(tv.string.suffix(12)))]")
      report(
        "quote continuation exit caret",
  ...
  @MainActor public enum EmptyDocScrollProbe {
