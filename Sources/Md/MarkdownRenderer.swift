...
public enum MarkdownRenderer {
    ...
    private static func base(_ font: MarkdownFont, default d: NSFont) -> NSFont {
        ...
        switch font.kind {
        case .code:
            f = NSFont.monospacedSystemFont(ofSize: font.size, weight: weight)
        default:
            f = d.withSize(font.size)
        }
        return withTraits(font, on: f)
    }
    ...
    private static func resolveUIKit(_ font: MarkdownFont) -> UIFont {
        ...
        default:
            f = .systemFont(ofSize: font.size, weight: weight)
        }
        guard !font.traits.isEmpty else { return f }
    ...
    public static func currentCodeScheme() -> CodeColorScheme {
        CodeColorScheme.systemAware
    }
    ...
    public static func render(_ source: NSAttributedString) -> NSAttributedString {
        ...
        out.enumerateAttributes(in: full, options: []) { attrs, range, _ in
            ...
            for (key, value) in attrs {
                switch value {
                case let c as MarkdownColor:
                    resolved[key] = resolve(c)
                case let f as MarkdownFont:
                    resolved[key] = resolve(f)
                case let p as MarkdownParagraph:
                    resolved[key] = resolve(p)
                default:
                    resolved[key] = value
                }
            }
