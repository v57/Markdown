...
public enum CheckboxRenderer {
    public static func image(checked: Bool, size: CGFloat = MarkdownMetrics.standard.checkboxImageSize) -> UIImage {
        if checked {
            ...
            UIImage(systemName: "circle")!
                .applyingSymbolConfiguration(
                    UIImage.SymbolConfiguration(pointSize: size, weight: .regular)
                        .applying(UIImage.SymbolConfiguration(paletteColors: [.systemBlue]))
                )!
        }
    }
}
