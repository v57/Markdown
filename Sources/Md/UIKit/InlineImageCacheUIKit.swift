  ...
  /// Cache of loaded inline images, keyed by URL. The editor layout manager draws
  /// these in place of ![alt](url) ranges; until an image loads, the alt text is
  /// shown. Nonisolated: reads happen on the main thread (layout/draw), writes on
  /// a private queue via `queue.sync` — mirrors `Sources/Md/InlineImageCache.swift`
  /// (AppKit) with UIImage.
  public final class InlineImageCache {
    ...
    public func image(for url: URL) -> UIImage? { images[url] }
