  ...
  @Suite struct ParserTests {
    ...
    @Test func blockquotes() {
      ...
      #expect(
        MarkdownParser.parse("> quote").attributed.attribute(
          .markdownBlockquote, at: 0, effectiveRange: nil) != nil)
    ...
    @Test func inlineBoldItalicStrike() {
      ...
      #expect(
        (b.attribute(.font, at: 2, effectiveRange: nil) as? NSFont)?.fontDescriptor.symbolicTraits
          .contains(.bold) == true)
      #expect(
        b.attribute(.markdownSyntax, at: 0, effectiveRange: nil) != nil
          && b.attribute(.markdownSyntax, at: 4, effectiveRange: nil) != nil)
      let ital = MarkdownParser.parse("*x*").attributed
      #expect(
        (ital.attribute(.font, at: 1, effectiveRange: nil) as? NSFont)?.fontDescriptor
          .symbolicTraits.contains(.italic) == true)
      let both = MarkdownParser.parse("***x***").attributed
      #expect(
        (both.attribute(.font, at: 3, effectiveRange: nil) as? NSFont)?.fontDescriptor
      ...
      #expect(
        strike.attribute(.strikethroughStyle, at: 2, effectiveRange: nil) as? Int
    ...
    @Test func inlineCodeChips() {
      ...
      #expect(
        (code.attribute(.font, at: 1, effectiveRange: nil) as? NSFont)?.fontName.contains("Mono")
          == true)
    ...
    @Test func doubleBacktickSpans() {
      ...
      #expect(
        (code2.attribute(.markdownCommandSpan, at: 3, effectiveRange: nil) as? NSValue)?.rangeValue
          == NSRange(location: 0, length: 7))
      #expect(
        code2.attribute(.markdownSyntax, at: 0, effectiveRange: nil) != nil
    ...
    @Test func twoInlineCodeChipsSeparate() {
      ...
      twoSpans.enumerateAttribute(
        .markdownInlineCode, in: NSRange(location: 0, length: twoSpans.length), options: []
      ) { value, range, _ in if value != nil { chipRuns.append(range) } }
    ...
    @Test func inlineEscapes() {
      ...
      #expect(
        [
          "**b** and *i* and `c` and ~~s~~ and \\*x\\*", "a ** b ** c", "emoji 🎉 *ok*",
          "# **bold heading**",
    ...
    @Test func lists() {
      ...
      #expect(
        ul.blocks == [
          .unorderedList(items: [.init(text: "a", level: 0), .init(text: "b", level: 0)], level: 0)
        ])
      #expect(ul.syntaxRanges.count == 2)
      let nested = MarkdownParser.parse("- a\n  - b")
      #expect(
        nested.blocks.first.map {
          if case .unorderedList(let items, _) = $0 { return items[1].level } else { return -1 }
        } == 1)
      let ol = MarkdownParser.parse("1. a\n2. b")
      #expect(
        ol.blocks == [
          .orderedList(items: [.init(text: "a", level: 0), .init(text: "b", level: 0)], level: 0)
        ])
      let task = MarkdownParser.parse("- [x] done\n- [ ] todo")
      #expect(
        task.blocks == [
          .taskList(items: [
            .init(text: "done", checked: true, level: 0),
            .init(text: "todo", checked: false, level: 0),
          ])
        ])
      #expect(
        task.attributed.attribute(.markdownCheckbox, at: 3, effectiveRange: nil) as? Bool == true)
      #expect(
        task.attributed.attribute(.markdownCheckbox, at: 14, effectiveRange: nil) as? Bool == false)
      #expect(
        ["- a\n- b", "- [x] done\n- [ ] todo", "1. a\n  2. b", "* i1\n* i2\n  * i2a"].allSatisfy {
          MarkdownParser.parse($0).attributed.string == $0
        })
    ...
    @Test func linksAndImages() {
      ...
      #expect(
        (img.attribute(.markdownImage, at: 2, effectiveRange: nil) as? URL)?.absoluteString
          == "https://a.b/c.png")
    ...
    @Test func tables() {
      ...
      #expect(
        t.blocks == [.table(header: ["a", "b"], rows: [["1", "2"]], alignments: [.left, .left])])
      ...
      #expect(
        (t.attributed.attribute(.font, at: 2, effectiveRange: nil) as? NSFont)?.fontDescriptor
          .symbolicTraits.contains(.bold) == true)
      #expect(
        (t.attributed.attribute(.font, at: 13, effectiveRange: nil) as? NSFont)?.fontName.contains(
          "Mono") == true)
      #expect(
        [
          "| a | b |\n|---|---|\n| 1 | 2 |", "a | b\n---|---\n1 | 2", "|x|y|\n|:-|-:|\n|1|2|",
    ...
    @Test func featureAttributes() {
      #expect(
        MarkdownParser.parse("---").attributed.attribute(.markdownRule, at: 0, effectiveRange: nil)
          != nil)
    }
    ...
    @Test func sampleDocument() {
      ...
      #expect(
        sd.blocks.filter { if case .heading = $0 { return true } else { return false } }.count == 2)
    ...
    @Test func zeroWidthHiddenCommands() {
      ...
      zwLM.invalidateGlyphs(
        forCharacterRange: NSRange(location: 0, length: 10), changeInLength: 0,
        actualCharacterRange: nil)
      zwLM.invalidateLayout(
        forCharacterRange: NSRange(location: 0, length: 10), actualCharacterRange: nil)
      zwLM.ensureLayout(for: zwContainer)
    ...
    @Test func newlineFragmentsAndZeroWidthGlyph() {
      ...
      let nlDoc = MarkdownParser.parse(
        "Hello" + String(UnicodeScalar(10)) + "Hello", style: MarkdownStyleSpec.standard
      ).attributed
      ...
      var nlFrags: [NSRange] = []
      nlLM.enumerateLineFragments(
        forGlyphRange: nlLM.glyphRange(
          forCharacterRange: NSRange(location: 0, length: nlStorage.length),
          actualCharacterRange: nil)
      ) { _, _, _, glyphRange, _ in nlFrags.append(glyphRange) }
      #expect(
        nlFrags.count == 2 && nlFrags[0] == NSRange(location: 0, length: 6)
          && nlFrags[1] == NSRange(location: 6, length: 5))
      // Font zero-advance glyph availability (for the ZWSP substitution fix)
      var sfHasZeroWidth = false
      for font in [
        NSFont.systemFont(ofSize: 15), NSFont.systemFont(ofSize: 20, weight: .semibold),
        ...
        for char in [0x200B, 0x2060, 0xFEFF] {
          ...
          if gl[0] != 0 {
            found.append("U+\(String(format: "%04X", char)) g=\(gl[0]) adv=\(adv[0].width)")
          }
        }
    ...
    @Test func headingHeightStable() {
      let hdDoc =
        MarkdownParser.parse("### Task list\nplain").attributed.mutableCopy()
      ...
      func headingHeight(activeRange: NSRange, invalidateRange: NSRange) -> CGFloat {
        ...
        hdLM.invalidateGlyphs(
          forCharacterRange: invalidateRange, changeInLength: 0, actualCharacterRange: nil)
        hdLM.invalidateLayout(forCharacterRange: invalidateRange, actualCharacterRange: nil)
        hdLM.ensureLayout(for: hdContainer)
        let glyphRange = hdLM.glyphRange(
          forCharacterRange: NSRange(location: 0, length: 11), actualCharacterRange: nil)
      ...
      let hInactive = headingHeight(
        activeRange: NSRange(location: 12, length: 5), invalidateRange: wholeDoc)  // caret on "plain"
      _ = headingHeight(activeRange: NSRange(location: 0, length: 11), invalidateRange: wholeDoc)
      let hActiveLineOnly = headingHeight(
        activeRange: NSRange(location: 0, length: 11), invalidateRange: headingLine)
    ...
    @Test func commandSpanAttributes() {
      ...
      #expect(
        (bs.attribute(.markdownCommandSpan, at: 0, effectiveRange: nil) as? NSValue)?.rangeValue
          == NSRange(location: 0, length: 8))
      #expect(
        (bs.attribute(.markdownCommandSpan, at: 3, effectiveRange: nil) as? NSValue)?.rangeValue
          == NSRange(location: 0, length: 8))
      #expect(
        (bs.attribute(.markdownCommandSpan, at: 14, effectiveRange: nil) as? NSValue)?.rangeValue
      ...
      #expect(
        (codeSpan.attribute(.markdownCommandSpan, at: 4, effectiveRange: nil) as? NSValue)?
          .rangeValue == NSRange(location: 2, length: 6))
      let linkSpan = MarkdownParser.parse("[t](https://a.b)").attributed
      #expect(
        (linkSpan.attribute(.markdownCommandSpan, at: 1, effectiveRange: nil) as? NSValue)?
          .rangeValue == NSRange(location: 0, length: 16))
    }
    ...
    @Test func inlineDelimitersShowWhenCaretInsideSpan() {
      ...
      let csDoc =
        MarkdownParser.parse("**bold** x").attributed.mutableCopy() as! NSMutableAttributedString
      ...
      func spanWidth(caret: Int) -> CGFloat {
        ...
        csLM.invalidateGlyphs(
          forCharacterRange: NSRange(location: 0, length: csStorage.length), changeInLength: 0,
          actualCharacterRange: nil)
        csLM.invalidateLayout(
          forCharacterRange: NSRange(location: 0, length: csStorage.length),
          actualCharacterRange: nil)
    ...
    @Test func listContinuations() {
      #expect(
        MarkdownParser.listContinuation(for: "- item")
          == MarkdownParser.ListContinuation(marker: "- ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "* item")
          == MarkdownParser.ListContinuation(marker: "* ", empty: false))
      #expect(
      ...
      #expect(
        MarkdownParser.listContinuation(for: "  - item")
          == MarkdownParser.ListContinuation(marker: "  - ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "1. item")
          == MarkdownParser.ListContinuation(marker: "2. ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "10) item")
          == MarkdownParser.ListContinuation(marker: "11) ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "- [ ] todo")
          == MarkdownParser.ListContinuation(marker: "- [ ] ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "- [x] done")
          == MarkdownParser.ListContinuation(marker: "- [x] ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "  - [x] done")
          == MarkdownParser.ListContinuation(marker: "  - [x] ", empty: false))
      #expect(
        MarkdownParser.listContinuation(for: "- ")
          == MarkdownParser.ListContinuation(marker: "- ", empty: true))
      #expect(
        MarkdownParser.listContinuation(for: "- [ ] ")
          == MarkdownParser.ListContinuation(marker: "- [ ] ", empty: true))
      #expect(
        MarkdownParser.listContinuation(for: "1.   ")
          == MarkdownParser.ListContinuation(marker: "2.   ", empty: true))
    ...
    @Test func quoteContinuations() {
      ...
      #expect(
        MarkdownParser.quoteContinuation(for: "> quoted")
          == MarkdownParser.ListContinuation(marker: "> ", empty: false))
      #expect(
        MarkdownParser.quoteContinuation(for: ">quoted")
          == MarkdownParser.ListContinuation(marker: "> ", empty: false))
      #expect(
        MarkdownParser.quoteContinuation(for: ">> deep")
          == MarkdownParser.ListContinuation(marker: ">> ", empty: false))
      #expect(
        MarkdownParser.quoteContinuation(for: "  > quoted")
          == MarkdownParser.ListContinuation(marker: "  > ", empty: false))
      #expect(
        MarkdownParser.quoteContinuation(for: "> ")
          == MarkdownParser.ListContinuation(marker: "> ", empty: true))
      #expect(
        MarkdownParser.quoteContinuation(for: ">   ")
          == MarkdownParser.ListContinuation(marker: ">   ", empty: true))
    ...
    @Test func swiftMarkdownRegressions() {
      ...
      #expect(
        bare.blocks == [
          MarkdownParser.Block.unorderedList(items: [.init(text: "[ ]", level: 0)], level: 0)
        ])
      #expect(
        MarkdownParser.parse("- [ ] ").blocks == [
          .taskList(items: [.init(text: "", checked: false, level: 0)])
        ])
      ...
      #expect(
        (ref.attributed.attribute(.link, at: 1, effectiveRange: nil) as? URL)?.absoluteString
          == "https://a.b")
      let auto = MarkdownParser.parse("<https://a.b>")
      #expect(
        (auto.attributed.attribute(.link, at: 2, effectiveRange: nil) as? URL)?.absoluteString
          == "https://a.b")
      #expect(
        auto.attributed.attribute(.markdownSyntax, at: 0, effectiveRange: nil) != nil
    ...
    @Test func quoteBarGeometryHeadless() {
      ...
      let qbGlyphRange = qbLM.glyphRange(
        forCharacterRange: NSRange(location: 0, length: qbStorage.length), actualCharacterRange: nil
      )
    ...
    @Test func mixedLineCodeChipNarrower() {
      ...
      let mixedDoc =
        MarkdownParser.parse("text `code` more").attributed.mutableCopy()
        as! NSMutableAttributedString
      ...
      mixedLM.enumerateLineFragments(
        forGlyphRange: mixedLM.glyphRange(
          forCharacterRange: NSRange(location: 0, length: mixedStorage.length),
          actualCharacterRange: nil)
      ) { rect, _, _, _, _ in mixedLineWidth = max(mixedLineWidth, rect.width) }
    ...
    @Test func highlightedFenceTokenColors() {
      withLightAppearance {
        ...
        func colorAt(_ word: String) -> NSColor? {
          ...
          return parsed.attributed.attribute(.foregroundColor, at: loc, effectiveRange: nil)
            as? NSColor
        }
        ...
        #expect(
          parsed.attributed.attribute(
            .underlineStyle, at: mdNs.range(of: "https://a.b").location, effectiveRange: nil)
            as? Int == NSUnderlineStyle.single.rawValue)
        #expect(
          parsed.attributed.attribute(
            .underlineColor, at: mdNs.range(of: "https://a.b").location, effectiveRange: nil)
            as? NSColor == ns(CodeColorScheme.githubLight.link))
        #expect(
          parsed.attributed.attribute(
            .markdownCodeBlock, at: mdNs.range(of: "func").location, effectiveRange: nil) != nil)
      }
    }
    ...
    @Test func codeLanguageLabel() {
      ...
      let langName =
        langDoc.attributed.attribute(
          .markdownCodeLanguage, at: ldNs.range(of: "let a").location, effectiveRange: nil)
        as? String
      #expect(langName == "Swift")
      let unknownLangDoc = MarkdownParser.parse("```xyz\nabc\n```")
      let ulNs = unknownLangDoc.attributed.string as NSString
      let ulName =
        unknownLangDoc.attributed.attribute(
          .markdownCodeLanguage, at: ulNs.range(of: "abc").location, effectiveRange: nil) as? String
      #expect(ulName == nil)
      #expect(
        langDoc.attributed.string == "```swift\nlet a = 1\n```"
    ...
    @Test func unknownLanguageAndTwoFences() {
      withLightAppearance {
        ...
        #expect(
          un.attributed.attribute(
            .foregroundColor, at: (unknown as NSString).range(of: "func").location,
        ...
        let pyNum =
          tp.attributed.attribute(
            .foregroundColor, at: tNs.range(of: "x = 1").location + 4, effectiveRange: nil)
          as? NSColor
        let jsKw =
          tp.attributed.attribute(
            .foregroundColor, at: tNs.range(of: "const").location, effectiveRange: nil) as? NSColor
