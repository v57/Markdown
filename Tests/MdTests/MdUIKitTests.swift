  ...
  @Suite struct MdUIKitTests {
    ...
    @Test func uikitStyleResolvesNativeTypes() {
      ...
      let h4Traits =
        style.headingUIFont(level: 4).fontDescriptor.fontAttributes[.traits]
    ...
    @Test func uikitParserOutputIsNative() {
      ...
      #expect(
        (a.attribute(.font, at: codeLoc, effectiveRange: nil) as? UIFont)?.fontName.contains("Mono")
          == true)
    ...
    @Test func newlineFragments() {
      ...
      nlLM.enumerateLineFragments(
        forGlyphRange: nlLM.glyphRange(
          ...
          actualCharacterRange: nil)
      ) { _, _, _, glyphRange, _ in frags.append(glyphRange) }
      #expect(
        frags.count == 2 && frags[0] == NSRange(location: 0, length: 6)
          && frags[1] == NSRange(location: 6, length: 5))
    }
